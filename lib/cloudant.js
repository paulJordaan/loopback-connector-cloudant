...
Cloudant.prototype.all = function(model, filter, callback) {
  ...
  function processResponse(err, result) {
    ...
    if (filter && filter.include) {
      self._models[model].model.include(results, filter.include, function(err, _results){
        debug("INCLUDE RESULT", JSON.stringify(_results[0].posts));
        return callback && callback(null, _results);
      });
    } else {
 ...
 * @param {Function} [callback] The callback function
 */
Cloudant.prototype.updateAttributes = function(model, id ,data, cb) {
  var self = this;
  ...
  self.db.get(id, function(err, doc) {
    ...
    self.db.save(model, helpers.merge(doc, data), function(err, rsp) {
      if (err) {
        return callback && callback(err);
      }
      doc._rev = rsp.rev;
